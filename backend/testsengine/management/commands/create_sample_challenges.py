...
class Command(BaseCommand):
 help = 'Crée des défis de codage de test'

 def handle(self, *args, **options):
 self.stdout.write('Création des défis de test...')

 # Créer le défi Palindrome
 palindrome_challenge, created = CodingChallenge.objects.get_or_create(
...
print(is_palindrome("A man a plan a canal Panama")) # True''',
 ...
 if created:
 self.stdout.write(self.style.SUCCESS(f' Créé: {palindrome_challenge.title}'))
 else:
 self.stdout.write(self.style.WARNING(f'️ Existe déjà: {palindrome_challenge.title}'))

 # Créer le défi Two Sum
 two_sum_challenge, created = CodingChallenge.objects.get_or_create(
 slug='two-sum',
 defaults={
...
print(two_sum([3, 2, 4], 6)) # [1, 2]''',
 ...
 if created:
 self.stdout.write(self.style.SUCCESS(f' Créé: {two_sum_challenge.title}'))
 else:
 self.stdout.write(self.style.WARNING(f'️ Existe déjà: {two_sum_challenge.title}'))

 total_count = CodingChallenge.objects.count()
 self.stdout.write(self.style.SUCCESS(f' Total: {total_count} défi(s) dans la base'))
